...
async def get_points(s: req_Session, number_c: int):
    if check_login_status(s, number_c):
        ...
        points = re.findall("积分: (\d+)", res.text)
        if len(points) != 0:
            user_points.append(points[0])  # 将积分存储到用户积分列表中
        else:
            user_points.append("未知")  # 如果无法获取积分，将其设为未知
        message = f"用户“{user_list[number_c - 1]}”已完成积分获取，当前积分：{user_points[0]}"
        print(message)  # 打印并发送消息
        await send_telegram_message(message)  # 发送 Telegram 消息
        return user_list[number_c - 1], user_points[0]  # 返回用户名和积分
    else:
        print("请检查你的帐户是否正确！")
...
if __name__ == "__main__":
    ...
    if not username or not password:
        ...
        user_points_list = []  # 用于存储所有用户的积分
        # 依次登录帐户获取积分，出现错误时不中断程序继续尝试下一个帐户
        loop = asyncio.get_event_loop()
        tasks = [get_points(login(user_list[i], passwd_list[i]), i + 1) for i in range(len(user_list))]
        user_points_list = loop.run_until_complete(asyncio.gather(*tasks))

        # 构建消息内容
        formatted_messages = []
        for user, points in user_points_list:
            formatted_messages.append(f"用户: {user}\n当前积分: {points}")
